...
<<AppendixK, TEST _Credit Card_ data>>.
...
[#BatchProcessingApi_Fields_Sepa]
==== _SEPA_ and _Credit Card_ Response
...
| request_id                     |  Mandatory  | Alphanumeric | 64   | Unique request ID for each Transaction (= Merchant Transaction ID)
...
_Wirecard Payment Gateway_ files should be provided in CSV format, EFT request
